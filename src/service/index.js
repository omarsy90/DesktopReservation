...
class Service {
  async userRegistering(user) {
    const response = await axios.post(`${basiUrl}/register`, {
      //ID: 12345
      email: user.email,
  ...
  async logIn(auth) {
    // this function return data (token and userId) if there is no error
    // return 400 if some of parametrer fehlt
    // return 401 if username or password is unvalid
  ...
  async getUserProfile() {
    // this function return user object or false if error come from api
    const userId =
  ...
  // this function return either object or number(400,409) if error happened
  async updateProfile(token, newInfo) {
  ...
  async getAlldesks() {
    // return array of object-desk  if request right
    // return 400 if parameter fehlt
    //return 401 if unautherized (token not more valid)
    const response = await axios
      .get(`${basiUrl}/desk`, {
        headers: {
          authorization: this.getToken(),
...
async filterDesks(from, to, stiegeId) {
  from = moment(from).format("YYYY-MM-DD");
  to = moment(to).format("YYYY-MM-DD");
 
  const allDesks = await this.getAlldesks();
  const bookings = await this.getFlexdeskBooking();
  ...
  for (let i = 0; i < allDesks.length; i++) {
    ...
    for (let j = 0; j < bookings.length; j++) {
      if (allDesks[i].id == bookings[j].desk.id) {

        if (from >= moment( bookings[j].dateStart).format("YYYY-MM-DD") && from <= moment( bookings[j].dateEnd).format("YYYY-MM-DD")) {
          isBooked = true;
          break;
        }

        if (to <= moment( bookings[j].dateEnd).format("YYYY-MM-DD") && to >= moment(bookings[j].dateStart).format("YYYY-MM-DD")) {
          isBooked = true;
          break;
        }
    ...
    if (!isBooked) {
      if (!stiegeId) {
        filteredDesks.push(allDesks[i]);
      } else if (allDesks[i].office.id == stiegeId) {
        filteredDesks.push(allDesks[i]);
      }
    }
  }
  ...
  // filter desks based on only stiege argument
